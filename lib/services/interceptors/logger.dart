...
/*
No 3rd party plugin
Purpose: The code helps with logging HTTP requests and responses in a Flutter application using the Dio HTTP client.

Log Levels: It defines log levels like debug, info, warning, error, and alien (ALIEN).
Logging Function: logDebug function logs messages at specified log levels and shows time (hours, minutes, seconds) with the messages. It applies different colors to log messages based on the log level.
Log Colors: The log messages are color-coded for better readability. Different colors are used for different log levels.
Debug Mode: Logging only works when the app is running in debug mode (kDebugMode). This ensures that logs are not displayed in production builds, avoiding performance overhead.
HTTP Request Logging: The LoggerInterceptor class logs HTTP request details like method, URL, headers, and request data (formatted as pretty JSON).
HTTP Response Logging: The LoggerInterceptor class logs HTTP response details, including status code and response data (formatted as pretty JSON).
HTTP Error Logging: The LoggerInterceptor class logs HTTP error details, including the error request and error message.
Long Response Handling: The code uses debugPrint to display complete response data without truncation.
JSON Data Formatting: It has a _prettyJsonEncode method to format data as pretty JSON for better readability in logs.
*/
